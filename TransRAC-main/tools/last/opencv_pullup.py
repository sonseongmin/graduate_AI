...
# --- 설정 ---
VIDEO_PATH = r"C:\mycla\TransRAC-main\RepCountA\video\train\stu5_29.mp4"  # 🎥 테스트할 영상 경로
EXERCISE = "pullup"

# 임계값 (풀업 동작 범위)
LOW_TH, HIGH_TH = 120, 138       # 팔꿈치 각도 기준 (낮을수록 올라간 상태)
DEBOUNCE_FR = 1
TARGET_RANGE = 90.0
...
with mp_pose.Pose(model_complexity=2,
    ...
    while True:
        ...
        if results.pose_landmarks:
            ...
            if vis >= MIN_VIS:
                ...
                if hold_low >= DEBOUNCE_FR:
                    stage = "up"      # 올라간 상태
                if stage == "up" and hold_high >= DEBOUNCE_FR:
                    stage = "down"   # 내려간 상태
                    count += 1
                    rep_min, rep_max = 999.0, -999.0
                    rep_vis_sum, rep_frames = 0.0, 0
        ...
        cv2.imshow("Pull-up Counter", cv2.resize(frame, (960, 720)))
