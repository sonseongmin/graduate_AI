...
def _jumpingjack_angle(landmarks):
    l_sh, l_hip, l_wr, l_ank = _get(landmarks, mp_pose.PoseLandmark.LEFT_SHOULDER), _get(landmarks, mp_pose.PoseLandmark.LEFT_HIP), _get(landmarks, mp_pose.PoseLandmark.LEFT_ELBOW), _get(landmarks, mp_pose.PoseLandmark.LEFT_WRIST), _get(landmarks, mp_pose.PoseLandmark.LEFT_ANKLE)
    r_sh, r_hip, r_wr, r_ank = _get(landmarks, mp_pose.PoseLandmark.RIGHT_SHOULDER), _get(landmarks, mp_pose.PoseLandmark.RIGHT_HIP), _get(landmarks, mp_pose.PoseLandmark.RIGHT_ELBOW), _get(landmarks, mp_pose.PoseLandmark.RIGHT_WRIST), _get(landmarks, mp_pose.PoseLandmark.RIGHT_ANKLE)
    la = _angle_3d(l_sh, l_hip, l_wr, l_ank)
    ra = _angle_3d(r_sh, r_hip, r_wr, r_ank)
    return None if la is None or ra is None else min(la, ra)
...
def analyze_video_run(video_path: str, exercise: str, pose_model) -> dict:
    ...
    if exercise not in {"pushup", "squat", "pullup", "jumpingjack"}:
    ...
    if exercise == "pushup":
        ...
        angle_fn = _pullup_angle
        down_thr, up_thr, strict_min, min_duration_s = 80, 150, 70, 0.3
        exercise_type = "pullup"
    elif exercise == "jumpingjack":
        angle_fn = _jumpingjack_angle
        down_thr, up_thr, strict_min, min_duration_s = 90, 150, 80, 0.3
        exercise_type = "jumpingjack"
