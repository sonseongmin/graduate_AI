...
def avg_shoulder_angle(lm):
    # 어깨-팔꿈치-손목 각도 (팔을 들어올릴 때 감소)
    left = (lm[mp_pose.PoseLandmark.LEFT_HIP],
            lm[mp_pose.PoseLandmark.LEFT_SHOULDER],
            lm[mp_pose.PoseLandmark.LEFT_ELBOW])
    right = (lm[mp_pose.PoseLandmark.RIGHT_HIP],
...
LOW_TH, HIGH_TH, DEBOUNCE_FR = 40, 80, 2  # 팔 내릴 때↑, 들 때↓
TARGET_RANGE = 40.0
...
with mp_pose.Pose(model_complexity=2, min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while True:
        ...
        if args.display:
            ...
            cv2.imshow("Front Raise Counter", cv2.resize(image, (960, 540)))
